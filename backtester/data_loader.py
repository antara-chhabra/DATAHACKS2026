...
class TickData:
    ...
    # Chainlink (per-asset; forward-filled each tick)
...
def _oracle_open_close_for_lifecycle(
    lc: MarketLifecycle,
    asset_cl: Any,
) -> tuple[float, float]:
    """
    Chainlink open/close aligned with settlement logic (±5s windows, else nearest).
    Returns (0, 0) if no usable data.
    """
    if asset_cl is None or getattr(asset_cl, "empty", True):
        return 0.0, 0.0
...
def compute_settlements(
    ...
    Known outcomes still get chainlink_open/close filled when oracle data exists
    (strategies should use these for fair value / direction, not first-tick CL).
    """
    ...
    for lc in lifecycles:
        ...
        if asset_cl is not None and not asset_cl.empty:
            open_price, close_price = _oracle_open_close_for_lifecycle(lc, asset_cl)
...
def build_timeline(
    ...
    # Pre-aggregate Chainlink to per-second, per asset (BTC/ETH/SOL)
    chainlink_by_sec: dict[str, dict[int, float]] = {
        "BTC": {},
        "ETH": {},
        "SOL": {},
    }
    if not chainlink_df.empty:
        if "symbol" in chainlink_df.columns:
            for sym, grp in chainlink_df.groupby("symbol", sort=False):
                base = str(sym).split("/")[0].upper()
                if base not in chainlink_by_sec:
                    continue
                g2 = grp.groupby("ts_sec")["price"].last()
                chainlink_by_sec[base].update(
                    {int(k): float(v) for k, v in g2.items()}
                )
        else:
            g2 = chainlink_df.groupby("ts_sec")["price"].last()
            chainlink_by_sec["BTC"].update(
                {int(k): float(v) for k, v in g2.items()}
            )
    ...
    last_btc_mid = 0.0
    last_btc_spread = 0.0
    last_chainlink_btc = 0.0
    last_chainlink_eth = 0.0
    last_chainlink_sol = 0.0
    ...
    for ts in range(global_start, global_end + 1):
        ...
        # Chainlink (forward-fill per asset)
        if ts in chainlink_by_sec["BTC"]:
            last_chainlink_btc = chainlink_by_sec["BTC"][ts]
        if ts in chainlink_by_sec["ETH"]:
            last_chainlink_eth = chainlink_by_sec["ETH"][ts]
        if ts in chainlink_by_sec["SOL"]:
            last_chainlink_sol = chainlink_by_sec["SOL"][ts]
        tick.chainlink_btc = last_chainlink_btc
        tick.chainlink_eth = last_chainlink_eth
        tick.chainlink_sol = last_chainlink_sol
