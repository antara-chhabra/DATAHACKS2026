...
├── my_strategy.py                  # PrismFV multi-signal strategy
├── max_pnl_hybrid.py / v2          # Hybrid P&L-optimised variants
├── analytics.py                    # Standalone analysis utilities
├── backtester/                     # Engine, scoring, strategy ABC
│   └── examples/                   # 4 example strategies
├── alphasee/                       # Interactive strategy dashboard (Next.js)
│   ├── app/                        # Next.js App Router pages
│   │   ├── globals.css             # Light + dark mode CSS variables
│   │   ├── ThemeContext.tsx        # Theme provider
│   │   ├── page.tsx                # Landing page (animated coin background)
│   │   └── dashboard/page.tsx      # Full analytics dashboard
│   ├── components/                 # All dashboard components
│   │   ├── FallingCoins.tsx        # Canvas-based animated coin rain
│   │   ├── ThemeToggle.tsx         # Dark / light mode toggle button
│   │   ├── EDA.tsx                 # Exploratory Data Analysis table
│   │   ├── StrategyBox.tsx         # Strategy rationale (data → decision)
│   │   ├── Volatility3D.tsx        # 3D Plotly volatility surface
│   │   ├── EnvironmentalImpact.tsx # BTC/ETH energy charts + insights
│   │   ├── CorrelationSignals.tsx  # Energy/macro & cross-market correlation
│   │   └── ...                     # DecisionEngine, Assets, Charts, Insights
...
There are three things you can run. Open separate terminal tabs for each.

### 1 — AlphaSee Dashboard (Next.js)
...
Open http://localhost:2718, or click **"Try it on Marimo / Sphinx"** on the dashboard.

> Requires the training data — run `python download_data.py` first if you haven't already.

### 3 — Python Backtester

```bash
pip install -r requirements.txt
python download_data.py              # download ~1.3 GB of data
python run_backtest.py my_strategy.py                        # training set
python run_backtest.py my_strategy.py --data data/validation/ # validation
```
...
The `alphasee/` directory contains our interactive strategy dashboard — a Next.js web app that visualises every layer of the strategy.

**Dashboard highlights:**
- Dark / light mode toggle (floating pill button, persists in `localStorage`)
- Animated miniature BTC / ETH / SOL coins falling in the background
- **EDA box** — clickable table of raw Polymarket order book insights
- **Strategy Rationale box** — each design decision traced back to a data observation
- **Probability Engine** — live Black-Scholes P(YES) calculator with sliders mirroring `my_strategy.py` exactly
- **3D Volatility Surface** — Plotly.js surface plot per coin showing implied vol across strike × time
- **Environmental Impact** — BTC and ETH energy consumption charts with clickable insight boxes and a mock energy-adjusted strategy return
- **Correlation panels** — Energy & Macro scatter + Cross-Market rolling correlation time series
- **"Try it on Marimo / Sphinx"** CTA — opens the live Marimo app at http://localhost:2718

See [`alphasee/README.md`](alphasee/README.md) for full component docs, tech stack, and project structure.

---

## Marimo Analytics App

`analytics.py` (authored by Jayani, from the `jayani` branch) is a fully reactive Marimo notebook that:
- Loads Polymarket SQLite + Chainlink oracle + Binance LOB data
- Runs Black-Scholes fair-value computation per tick (same math as `my_strategy.py`)
- Analyses arbitrage edge across all 8,466 binary markets
- Visualises win rates, pricing bias, and order book depth per asset and interval

Run with `marimo run analytics.py --port 2718`.
