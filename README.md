...
AlphaSee is an interactive trading strategy dashboard built for the DATAHACKS 2026 Economics & Data Analytics track. It visualizes a multi-signal strategy that trades BTC/ETH/SOL binary prediction markets on Polymarket, using Chainlink oracles, Binance order book data, and cross-asset correlation to identify high-confidence entry points.
...
| Validation (out-of-sample) | +23.0% | 16.36 | 48.6% | 15.3% |

---

## Strategy — `AggressiveEdgeV3`

The strategy (`my_strategy.py`) combines two distinct signal types:

**1. Arbitrage**
Whenever `YES ask + NO ask < $1.00`, both sides are purchased simultaneously for a guaranteed profit. Up to 8 entries per market, capped at 240 shares each.

**2. Directional**
Trades BTC and ETH prediction markets (SOL disabled — too noisy) using a 4-signal gate:

| Signal | Source | Role |
|--------|--------|------|
| Real price drift | Chainlink oracle | Ground truth vs. market open |
| Momentum | Binance mid-price (20-tick) | Direction confirmation |
| Market edge | Polymarket ask vs. fair value | Opportunity sizing |
| Cross-asset agreement | BTC/ETH correlation | Confidence filter |

A Black-Scholes-inspired fair probability `_fair_prob()` uses per-asset, per-interval volatility calibrations. Trades only execute when edge exceeds a minimum threshold (4.5¢), adjusted ±1¢ based on cross-asset agreement.

**Key parameters:**
```python
arb_edge = 0.006          # min arbitrage gap to enter
min_edge = 0.045          # min directional edge
momentum_threshold = 0.0002
cash_frac = 0.92          # max cash to deploy
...
The AlphaSee dashboard (`alphasee/`) is a Next.js app that visualizes the strategy and its results interactively.
...
- **Landing page** — strategy summary with key metrics and crypto coin visuals
- **Decision Engine** — interactive signal cards; toggle each signal and see the trade decision update live
- **Why This Strategy** — plain-language explanation of the multi-signal approach
- **Assets** — click BTC, ETH, or SOL to see its role, P&L, Sharpe, and win rate
- **Market Signals** — YES/NO probability bars across 5m, 15m, and 1h markets
- **Charts** — real backtest time-series data; toggle 20h/40h/80h/160h windows, and switch between aggregated and all 9 individual asset/interval lines
- **Insights** — 6 data-driven findings extracted directly from backtest results

### Tech

- Next.js 16 (App Router) · TypeScript · CSS Modules
- Recharts for charting
- Data sourced from `results/timeseries.json` (generated by backtests)
...
```
DATAHACKS2026/
├── my_strategy.py          # AggressiveEdgeV3 — submitted strategy
├── run_backtest.py         # Backtest runner
├── results/                # All backtest outputs + timeseries.json
│   ├── backtest_20h.txt
│   ├── backtest_40h.txt
│   ├── backtest_80h.txt
│   ├── backtest_160h.txt
│   ├── backtest_full.txt
│   ├── backtest_validation.txt
│   ├── backtest_BTC_5m.txt
│   ├── backtest_ETH_5m.txt
│   ├── backtest_SOL_5m.txt
│   ├── backtest_BTC_15m.txt
│   ├── backtest_ETH_15m.txt
│   ├── backtest_SOL_15m.txt
│   ├── backtest_BTC_hourly.txt
│   ├── backtest_ETH_hourly.txt
│   ├── backtest_SOL_hourly.txt
│   └── timeseries.json     # Parsed portfolio time-series for charts
├── alphasee/               # AlphaSee Next.js dashboard
│   ├── app/
│   │   ├── page.tsx        # Landing page
│   │   └── dashboard/      # Dashboard (6 sections)
│   ├── components/         # DecisionEngine, Charts, Assets, etc.
│   └── public/             # Static assets (SVG coin icons, data)
├── data/                   # Train + validation market data
├── backtester/             # Engine, strategy ABC, examples
├── docs/                   # DATA_FORMAT, SCORING, RULES
└── requirements.txt
```
